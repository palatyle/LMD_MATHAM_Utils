...
def write_profiles(profile, prof_name, directory):
    ...
    with open(filename,'w') as fout:
        ...
        fout.write('{:18}\n'.format('CO2 - 95%, N2 - 5%'))
        fout.write(' {:7}  {:13} {:14} {:7} {:7} {:13}\n'.format('HEIGHTS','TEMPERATURES','REL. HUMIDITY','U-WIND','V-WIND','DRY PRESSURE'))
        fout.write('{:>8} {:>13} {:>14} {:>7} {:>7} {:>13}\n'.format('[M]','[C]','[%]','[M/S]','[M/S]','[Pa]'))
        fout.write('\n')

        for line in np.transpose(profile):
            fout.write('{:8.1f}      {:8.2f}      {:9.5f}  {:6.2f}  {:6.2f}  {:12.4e}'.format(*line))
            fout.write('\n')


# Datafile
dir = '/home/palatyle/LMD_gen/trunk/test_run/'
fn = dir+'diagfi.nc'
...
for index, row in volc_df.iterrows():
    ...
    spr_profiles = np.vstack((alt_profile, temp_prof_spr, RH_prof_spr,u_prof_spr,v_prof_spr,pres_prof_spr))
    ...
    sum_profiles = np.vstack((alt_profile, temp_prof_sum, RH_prof_sum,u_prof_sum,v_prof_sum,pres_prof_sum))
    ...
    fall_profiles = np.vstack((alt_profile, temp_prof_fall, RH_prof_fall,u_prof_fall,v_prof_fall,pres_prof_fall))
    ...
    win_profiles = np.vstack((alt_profile[:], temp_prof_win[:], RH_prof_win[:],u_prof_win[:],v_prof_win[:],pres_prof_win[:]))

    print("write data")
    write_profiles(spr_profiles, volc_name+'_spring_cd', out)
    write_profiles(sum_profiles, volc_name+'_summer_cd', out)
    write_profiles(fall_profiles, volc_name+'_fall_cd', out)
    write_profiles(win_profiles, volc_name+'_winter_cd', out)
